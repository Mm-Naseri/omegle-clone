    ...
    if (usernameValue) {
      ...
      .then(response => {
        if (response.ok) {
          ...
          console.log('Username update request succeeded. New username:', usernameValue);
          const overlay = document.getElementById('username-update-overlay');
          if (overlay) {
            overlay.remove();
          }
        } else {
          console.error('Username update failed on server.');
        }
      })
      .catch(console.error);
    }
...
// Add click event to copy Room ID
copyRoomIdButton.addEventListener('click', () => {
  const roomId = roomIdText.innerText.replace('Room ID: ', '').trim();
